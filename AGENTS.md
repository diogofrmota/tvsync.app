...
- Objection (GDPR Art. 21) and the CCPA opt-out are one switch: `profiles.analytics_opt_out`, saved from Privacy Choices in Edit Profile so it follows the account across devices.
...
Movie and TV show detail pages lead with a compact header: a small poster column (`7.5rem` on mobile, `13rem` from `md`) beside the title, the rating directly under the title, then the fact badges and the genres. On TV shows the episode tracker comes next, then the "Your TV Show" panel; on movies the "Your movie" panel follows the header directly. The description, trailer, director, cast, and TMDB member reviews close both pages.
