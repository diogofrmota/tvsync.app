...
test('Profile renders exact information, social navigation, non-scrolling stats, and favourites', async () => {
  ...
  // Four compact icon-and-number counters fit in the restored profile section.
  assert.match(page, /<ProfileSummaryStats statistics=\{statistics\} \/>/);
  ...
  assert.match(headerActions, /FiSettings/);
  assert.doesNotMatch(
    headerActions,
    /FiShare2|FiBarChart2|Share profile|View statistics/
  );
